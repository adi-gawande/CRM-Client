...
const PaymentModePage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await put(`/payment-mode/${editing._id}`, { name });
        toast.success("Updated");
      } else {
        await post("/payment-mode", { name });
        toast.success("Added");
      }
      setOpenDialog(false);
      setName("");
  ...
  const confirmBulkSave = async () => {
    ...
    try {
      for (const d of bulkData) {
        await put(`/payment-mode/${d._id}`, { name: d.name });
      }
  ...
  const initiateBulkAdd = () => {
    ...
    if (!lines.length) {
      toast.error("No payment modes entered");
      return;
    }
  ...
  const confirmBulkAdd = async () => {
    ...
    try {
      ...
      toast.success("Bulk payment modes added");
  ...
  const confirmSingleDelete = async () => {
    await del(`/payment-mode/${deleteTarget._id}`);
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between mb-4">
        <h2 className="text-xl font-semibold">Payment Mode Master</h2>
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setOpenDialog(true)}>Add Payment Mode</Button>
        </div>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          {loading ? (
            ...
            <TableRow>
              <TableCell colSpan={4}>No payment modes found.</TableCell>
            </TableRow>
          ) : (
            data.map((d, i) => (
              <TableRow
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle>
              {editing ? "Edit Payment Mode" : "Add Payment Mode"}
            </DialogTitle>
          </DialogHeader>

          <div className="space-y-2 mt-2">
            <Input
              placeholder="Payment mode (e.g. Cash, UPI)"
              value={name}
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Delete {selected.length} selected payment modes?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Add {bulkLines.length} payment modes?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
