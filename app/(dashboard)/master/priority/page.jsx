...
const PriorityPage = () => {
  ...
  const fetchPrioritys = async () => {
    setLoading(true);
    try {
      const res = await get("/priority");
      setPrioritys(Array.isArray(res) ? res : []);
    } catch (err) {
      console.error(err);
      setPrioritys([]);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingPriority) {
        await put(`/priority/${editingPriority._id}`, {
          name: priorityName,
        });
        toast.success("Priority updated");
      } else {
        await post("/priority", { name: priorityName });
        toast.success("Priority added");
      }
      setPriorityName("");
      setEditingPriority(null);
      setOpenDialog(false);
      await fetchPrioritys();
    } catch (err) {
      console.error(err);
  ...
  const handleEdit = (g) => {
    setEditingPriority(g);
    setPriorityName(g.name);
    setOpenDialog(true);
  };
  ...
  const confirmSingleDelete = async () => {
    ...
    try {
      await del(`/priority/${deleteTarget._id}`);
  ...
  const initiateBulkDelete = () => {
    if (selected.length === 0) {
      toast.error("Please select at least one priority to delete.");
      return;
    }
  ...
  const confirmBulkDelete = async () => {
    try {
      for (const id of selected) {
        ...
        await del(`/priority/${id}`);
      }
      setSelected([]);
      setSelectAll(false);
      await fetchPrioritys();
      toast.success("Selected prioritys deleted");
    } catch (err) {
      console.error(err);
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelected(prioritys.map((p) => p._id));
      setSelectAll(true);
    }
  };
  ...
  const confirmBulkSave = async () => {
    ...
    try {
      for (const d of bulkData) {
        // eslint-disable-next-line no-await-in-loop
        await put(`/priority/${d._id}`, { name: d.name });
      }
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Priority Master</h2>
        <div className="flex gap-2">
          ...
          <Button onClick={() => setOpenDialog(true)}>Add Priority</Button>
        </div>
      </div>
      ...
      >
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Bulk Add Prioritys</DialogTitle>
          </DialogHeader>

          <div className="mt-2 space-y-2">
            <p className="text-sm text-muted-foreground">
              Paste a list of priority names below — one per line.
            </p>
            <textarea
              ...
              >
                <TableCell className="text-left">
                  <Checkbox
                    ...
                    checked={selected.includes(priority._id)}
                    onCheckedChange={() => handleSelect(priority._id)}
                ...
                <TableCell className="text-left">
                  {isBulkEditing ? (
                    <Input
                      value={
                        bulkData.find((item) => item._id === priority._id)
                          ?.name || ""
                      }
                      onChange={(e) => {
                ...
                <TableCell className="text-left">
                  <div className="flex gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="destructive"
                      onClick={() => initiateDelete(priority)}
                      disabled={bulkSaving || bulkAdding}
                    >
                      Delete
                    </Button>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <DialogFooter className="mt-4 flex justify-end gap-2">
            ...
            <Button onClick={handleSave} disabled={saving}>
              {saving ? (
                <Spinner>
                  {editingPriority ? "Updating..." : "Adding..."}
                </Spinner>
              ) : editingPriority ? (
                "Update"
              ) : (
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Selected Prioritys</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{selected.length}</strong>{" "}
              selected priority{selected.length > 1 ? "s" : ""}? This action
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Add <strong>{bulkAddCount}</strong> new priority
              {bulkAddCount > 1 ? "s" : ""}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
