...
const SectorPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingSector) {
        await put(`/sector/${editingSector._id}`, {
          name: sectorName,
        });
        toast.success("Sector updated");
      } else {
        await post("/sector", { name: sectorName });
  ...
  const confirmSingleDelete = async () => {
    ...
    try {
      await del(`/sector/${deleteTarget._id}`);
  ...
  const initiateBulkDelete = () => {
    if (selected.length === 0) {
      toast.error("Please select at least one sector to delete.");
      return;
    }
  ...
  const confirmBulkDelete = async () => {
    try {
      ...
      setSelectAll(false);
      await fetchSectors();
      toast.success("Selected sectors deleted");
    } catch (err) {
      console.error(err);
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelected(sectors.map((p) => p._id));
      setSelectAll(true);
    }
  };
  ...
  const confirmBulkSave = async () => {
    ...
    try {
      for (const d of bulkData) {
        // eslint-disable-next-line no-await-in-loop
        await put(`/sector/${d._id}`, { name: d.name });
      }
  ...
  const initiateBulkAdd = () => {
    ...
    if (lines.length === 0) {
      toast.error("Please enter at least one sector.");
      return;
    }
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Sector Master</h2>
        <div className="flex gap-2">
          ...
          <Button onClick={() => setOpenDialog(true)}>Add Sector</Button>
        </div>
      </div>
      ...
      >
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Bulk Add Sectors</DialogTitle>
          </DialogHeader>

          <div className="mt-2 space-y-2">
            <p className="text-sm text-muted-foreground">
              Paste a list of sector names below — one per line.
            </p>
            <textarea
              ...
              >
                <TableCell className="text-left">
                  <Checkbox
                    ...
                    checked={selected.includes(sector._id)}
                    onCheckedChange={() => handleSelect(sector._id)}
                ...
                <TableCell className="text-left">
                  {isBulkEditing ? (
                    <Input
                      value={
                        bulkData.find((item) => item._id === sector._id)
                          ?.name || ""
                      }
                      onChange={(e) => {
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <DialogFooter className="mt-4 flex justify-end gap-2">
            ...
            <Button onClick={handleSave} disabled={saving}>
              {saving ? (
                <Spinner>{editingSector ? "Updating..." : "Adding..."}</Spinner>
              ) : editingSector ? (
                "Update"
              ) : (
                "Add"
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Selected Sectors</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{selected.length}</strong>{" "}
              selected sector{selected.length > 1 ? "s" : ""}? This action will
              soft-delete them.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Add <strong>{bulkAddCount}</strong> new sector
              {bulkAddCount > 1 ? "s" : ""}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
