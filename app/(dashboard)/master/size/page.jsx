...
const SizePage = () => {
  ...
  const fetchSizes = async () => {
    setLoading(true);
    try {
      const res = await get("/size");
      setSizes(Array.isArray(res) ? res : []);
    } catch (err) {
      console.error(err);
      setSizes([]);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingSize) {
        await put(`/size/${editingSize._id}`, {
          name: sizeName,
        });
        toast.success("Size updated");
      } else {
        await post("/size", { name: sizeName });
  ...
  const confirmSingleDelete = async () => {
    ...
    try {
      await del(`/size/${deleteTarget._id}`);
  ...
  const initiateBulkDelete = () => {
    if (selected.length === 0) {
      toast.error("Please select at least one size to delete.");
      return;
    }
  ...
  const confirmBulkSave = async () => {
    ...
    try {
      for (const d of bulkData) {
        // eslint-disable-next-line no-await-in-loop
        await put(`/size/${d._id}`, { name: d.name });
      }
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Size Master</h2>
        <div className="flex gap-2">
          ...
          <Button onClick={() => setOpenDialog(true)}>Add Size</Button>
        </div>
      </div>
      ...
      >
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Bulk Add Sizes</DialogTitle>
          </DialogHeader>

          <div className="mt-2 space-y-2">
            <p className="text-sm text-muted-foreground">
              Paste a list of size names below — one per line.
            </p>
            <textarea
              ...
              >
                <TableCell className="text-left">
                  <Checkbox
                    ...
                    checked={selected.includes(size._id)}
                ...
                <TableCell className="text-left">
                  {isBulkEditing ? (
                    <Input
                      value={
                        bulkData.find((item) => item._id === size._id)?.name ||
                        ""
                      }
                      onChange={(e) => {
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <DialogFooter className="mt-4 flex justify-end gap-2">
            ...
            <Button onClick={handleSave} disabled={saving}>
              {saving ? (
                <Spinner>{editingSize ? "Updating..." : "Adding..."}</Spinner>
              ) : editingSize ? (
                "Update"
              ) : (
                "Add"
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Selected Sizes</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{selected.length}</strong>{" "}
              selected size{selected.length > 1 ? "s" : ""}? This action will
              soft-delete them.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Add <strong>{bulkAddCount}</strong> new size
              {bulkAddCount > 1 ? "s" : ""}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
