...
const LeadSourcePage = () => {
  ...
  const fetchLeadSources = async () => {
    ...
    setLoading(true);
    try {
      const res = await get(`/lead-source?companyId=${companyId}`);
      setLeadSources(Array.isArray(res) ? res : []);
    } catch (err) {
      console.error(err);
      setLeadSources([]);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingType) {
        await put(`/lead-source/${editingType._id}`, {
          leadSource,
          companyId,
        });
        toast.success("Lead source updated");
      } else {
        await post("/lead-source", {
          leadSource,
          companyId,
        });
  ...
  const confirmSingleDelete = async () => {
    ...
    try {
      await del(`/lead-source/${deleteTarget._id}`, { companyId });
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        selected.map((id) => del(`/lead-source/${id}`, { companyId }))
      );
      setSelected([]);
      setSelectAll(false);
      await fetchLeadSources();
      toast.success("Selected lead sources deleted");
    } catch (err) {
      console.error(err);
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelected(leadSources.map((t) => t._id));
      setSelectAll(true);
    }
  };
  ...
  const initiateBulkSave = () => {
    ...
    const invalid = bulkData.some((d) => !d.leadSource?.trim());

    if (invalid) {
      toast.error("Lead source must be filled before saving.");
      return;
    }
    ...
    <div className="p-4">
          ...
          {leadSources.length > 0 && (
            <Button
              ...
              onClick={() => {
                ...
                setBulkData(leadSources.map((d) => ({ ...d })));
              }}
            >
        ...
        <Table className="table-fixed w-full min-w-[600px]">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-[300px]">Lead Source</TableHead>
                ...
                >
                  ...
                  <TableCell>
                    {isBulkEditing ? (
                      <Input
                        value={
                          bulkData.find((item) => item._id === type._id)
                            ?.leadSource || ""
                        }
                        onChange={(e) => {
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle>
              {editingType ? "Edit Lead Source" : "Add Lead Source"}
            </DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter className="mt-4 flex justify-end gap-2">
            ...
            <Button onClick={handleSave} disabled={saving}>
              {saving ? (
                <Spinner>{editingType ? "Updating..." : "Adding..."}</Spinner>
              ) : editingType ? (
                "Update"
              ) : (
                "Add"
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{deleteTarget?.leadSource}</strong>? This action can be
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Selected Lead Sources</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{selected.length}</strong>{" "}
              selected lead source{selected.length > 1 ? "s" : ""}? This action
